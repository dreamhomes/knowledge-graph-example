...
def extract(page_dir, executive_csv):
    """
    @param: page_dir: network pages dir
    @param: executive_csv: save_path
    """
    pages = map(lambda _: os.path.join(page_dir, _), os.listdir(page_dir))
    pages = filter(lambda _: _.endswith('html'), pages)
    headers = ['name', 'gender', 'age', 'code', 'jobs']

    with open(executive_csv, 'w', encoding='utf-8', newline='') as file_directors:
        ...
        for page in pages:
            ...
            print(file_name)
            code = file_name.split('.')[0].split('\\')[-1]
            executives = []
